  ...
  return (
    <div 
      className={`group bg-white rounded-lg shadow-md p-8 transition-all duration-300 border border-gray-100 h-full flex flex-col ${
        path ? 'hover:shadow-xl hover:-translate-y-1 cursor-pointer' : 'hover:shadow-xl hover:-translate-y-1'
      }`}
    ...
    >
      <div className="relative">
        <div className={`w-16 h-16 bg-lawn-50 rounded-lg flex items-center justify-center text-lawn-600 mb-6 transition-all duration-300 transform ${
          'group-hover:bg-lawn-500 group-hover:text-white group-hover:scale-110'
        }`}>
          {icon}
        </div>
        {isHovered && path && (
      ...
      <h3 className={`text-xl font-semibold mb-4 group-hover:text-lawn-600 transition-colors duration-300`}>
        {title}
      </h3>
